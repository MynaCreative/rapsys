...
use App\Repositories\OpsPlan as Repository;
use App\Models\OpsPlan as Model;

use App\Http\Requests\OpsPlan\{
    Show    as ShowRequest,
...
class OpsPlanController extends Controller
{
    private Repository  $repository;

    private $module = 'Master';
    private $page = 'OpsPlan';
     ...
     * @param   Model $ops_plan
     * 
     * @return  ApiResponse
     * @throws  Throwable
     */
    public function show(ShowRequest $request, Model $ops_plan)
    {
        try {
            $data = $this->repository::init($ops_plan)->show($request);
     ...
     * @throws  Throwable
     */
    public function update(UpdateRequest $request, Model $ops_plan)
    {
        try {
            $this->repository::init($ops_plan)->update($request);
        } catch (Throwable $exception) {
            return redirect()->back()->withErrors([
                'error' => __('messages.error.internal_server'),
                'exception' => $exception->getMessage()
     ...
     * @throws  Throwable
     */
    public function destroy(DestroyRequest $request, Model $ops_plan)
    {
        try {
            $this->repository::init($ops_plan)->delete();
        } catch (Throwable $exception) {
            return redirect()->back()->withErrors([
                'error' => __('messages.error.internal_server'),
                'exception' => $exception->getMessage()
     ...
     * @throws  Throwable
     */
    public function restore(DestroyRequest $request, Model $ops_plan)
    {
        try {
            $this->repository::init($ops_plan)->restore();
        } catch (Throwable $exception) {
            return redirect()->back()->withErrors([
                'error' => __('messages.error.internal_server'),
                'exception' => $exception->getMessage()
