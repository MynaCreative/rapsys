...
use App\Repositories\Workflow as Repository;
use App\Models\Workflow as Model;

use App\Http\Requests\Workflow\{
    Show    as ShowRequest,
...
class WorkflowController extends Controller
{
    ...
    private $module = 'Setting/Configuration';
    private $page = 'Workflow';
     ...
     * @throws  Throwable
     */
    public function show(ShowRequest $request, Model $workflow)
    {
        try {
            $data = $this->repository::init($workflow)->show($request);
     ...
     * @throws  Throwable
     */
    public function update(UpdateRequest $request, Model $workflow)
    {
        try {
            $this->repository::init($workflow)->update($request);
        } catch (Throwable $exception) {
            return redirect()->back()->withErrors([
                'error' => __('messages.error.internal_server'),
                'exception' => $exception->getMessage()
     ...
     * @throws  Throwable
     */
    public function destroy(DestroyRequest $request, Model $workflow)
    {
        try {
            $this->repository::init($workflow)->delete();
        } catch (Throwable $exception) {
            return redirect()->back()->withErrors([
                'error' => __('messages.error.internal_server'),
                'exception' => $exception->getMessage()
     ...
     * @throws  Throwable
     */
    public function restore(DestroyRequest $request, Model $workflow)
    {
        try {
            $this->repository::init($workflow)->restore();
        } catch (Throwable $exception) {
            return redirect()->back()->withErrors([
                'error' => __('messages.error.internal_server'),
                'exception' => $exception->getMessage()
