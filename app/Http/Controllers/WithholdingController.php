...
use App\Repositories\Withholding as Repository;
use App\Models\Withholding as Model;

use App\Http\Requests\Withholding\{
    Show    as ShowRequest,
...
class WithholdingController extends Controller
{
    ...
    public function __construct(Repository $repository)
    {
        ...
        $this->middleware('permission:withholding');
    }
     ...
     * @param   Model $withholding
     * 
     * @return  ApiResponse
     * @throws  Throwable
     */
    public function show(ShowRequest $request, Model $withholding)
    {
        try {
            $data = $this->repository::init($withholding)->show($request);
     ...
     * @throws  Throwable
     */
    public function update(UpdateRequest $request, Model $withholding)
    {
        try {
            $this->repository::init($withholding)->update($request);
        } catch (Throwable $exception) {
            return redirect()->back()->withErrors([
                'error' => __('messages.error.internal_server'),
                'exception' => $exception->getMessage()
     ...
     * @throws  Throwable
     */
    public function destroy(DestroyRequest $request, Model $withholding)
    {
        try {
            $this->repository::init($withholding)->delete();
        } catch (Throwable $exception) {
            return redirect()->back()->withErrors([
                'error' => __('messages.error.internal_server'),
                'exception' => $exception->getMessage()
     ...
     * @throws  Throwable
     */
    public function restore(DestroyRequest $request, Model $withholding)
    {
        try {
            $this->repository::init($withholding)->restore();
        } catch (Throwable $exception) {
            return redirect()->back()->withErrors([
                'error' => __('messages.error.internal_server'),
                'exception' => $exception->getMessage()
